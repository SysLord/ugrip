...
- **CORS proxy setup**: `docker/cors-anywhere.js` wraps the `cors-anywhere` npm package; `docker/start.sh` runs it (`npm run cors`) alongside the built app (`npm run serve`) as two processes in the same container (ports 5001 and 5000). The `corsproxy/` directory is a separate, incomplete attempt at a custom Express-based proxy — it has no server code, just a `package.json`; treat it as dead/unused (see disclaimer in README.md).
- **Env vars**: `REACT_APP_CORS_SERVER` (client-side, baked in at build time — prefixed to the fetched URL) and `CORS_SERVER` (host:port the proxy itself binds to). Set via `.env` locally, or `--build-arg CORS_SERVER=...` / Dockerfile `ARG`/`ENV` for Docker builds.
